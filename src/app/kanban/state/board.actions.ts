...
export const BoardActions = createActionGroup({
  ...
  'Update Card': props<{ cardId: string; changes: Partial<{ title: string; description: string; dueDate: string; priority: Priority; tags: string[]; completed: boolean }> }>(),
  ...
  'Clear Last Deleted': emptyProps(),
    'Move Card': props<{ cardId: string; fromColumnId: string; toColumnId: string; toIndex: number }>(),
