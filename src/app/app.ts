...
import { SideNavComponent } from './ui/navigation/side-nav.component';
import { ToastContainerComponent } from './ui/dialogs/toast-container/toast-container.component';
import { Store } from '@ngrx/store';
import { AuthActions } from './ui/auth/state/auth.actions';
import { NotificationBellComponent } from './ui/notifications/components/notifications/notification-bell.component';
...
export class App {
  ...
  constructor() {
    ...
    try {
      const pref = localStorage.getItem('nav:collapsed');
      if (pref === '1') this.sidebarCollapsed.set(true);
    } catch {}
    effect(() => {
      try {
        localStorage.setItem('nav:collapsed', this.sidebarCollapsed() ? '1' : '0');
      } catch {}
    });
  }
  ...
  setAccent(c: string) {
    this.theme.setAccent(c);
  }
  closeSidebarOnMobile() {
    if (window.matchMedia('(max-width: 1024px)').matches) this.closeSidebar();
  }
