...
export class BoardsDashboardComponent implements OnDestroy {
  ...
  async rename(b: any) {
    const ref = this.dialog.open(InputDialogComponent, { data: { title: 'Rename board', label: 'Title', value: b.title } });
    const t = await ref.afterClosed;
    if (t) this.store.dispatch(BoardsActions.renameBoard({ boardId: b.id, title: t }));
  }
  toggleFavorite(id: string) {
    this.store.dispatch(BoardsActions.toggleFavorite({ boardId: id }));
  }
  async delete(id: string) {
    const ref = this.dialog.open(ConfirmDialogComponent, { data: { title: 'Delete board?', message: 'This will remove the board and its cards.' } });
    const ok = await ref.afterClosed;
    if (ok) this.store.dispatch(BoardsActions.deleteBoard({ boardId: id }));
  }
  undo() {
