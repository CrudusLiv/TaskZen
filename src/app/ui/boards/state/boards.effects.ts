...
import { getFirestore, collection, query, where, onSnapshot, addDoc, serverTimestamp, updateDoc, doc, deleteDoc, getDoc, getDocs, writeBatch, DocumentReference, QueryDocumentSnapshot } from 'firebase/firestore';
  ...
  deleteBoard$ = createEffect(()=> this.actions$.pipe(
    ...
    switchMap(({ boardId })=> this.user$.pipe(
      switchMap(user => {
        ...
        return from((async()=>{
          // Gather columns
          const colsSnap = await getDocs(query(collection(db,'boardColumns'), where('boardId','==', boardId))).catch(()=>undefined);
          // Gather cards
          const cardsSnap = await getDocs(query(collection(db,'cards'), where('boardId','==', boardId))).catch(()=>undefined);
          // Batch delete cards and columns in chunks (max 500 ops per batch)
          const refs: DocumentReference[] = [];
          if(cardsSnap) cardsSnap.forEach((d: QueryDocumentSnapshot)=> refs.push(d.ref));
          if(colsSnap) colsSnap.forEach((d: QueryDocumentSnapshot)=> refs.push(d.ref));
          // Push board doc last
          refs.push(doc(db,'boards',boardId));
          while(refs.length){
            const batch = writeBatch(db);
            refs.splice(0, 450).forEach(r=> batch.delete(r));
            await batch.commit();
          }
        })());
      })
