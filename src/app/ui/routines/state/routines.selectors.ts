...
const _selectRoutineByIdCache = new Map<
  string,
  MemoizedSelector<Record<string, unknown>, RoutineEntity | null>
>();
...
export const selectRoutineById = (
  ...
  if (!_selectRoutineByIdCache.has(id)) {
    _selectRoutineByIdCache.set(
      id,
      createSelector(
        selectRoutineEntities,
        (entities): RoutineEntity | null => entities[id] ?? null,
      ) as MemoizedSelector<Record<string, unknown>, RoutineEntity | null>,
    );
  }
